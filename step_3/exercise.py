...
class Ball(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        # We have to detect when the ball 
        rebound = Signal()
        ############################################################################################
        #                                                                                          #
        #                                 Write your HDL here                                      #
        #                                                                                          #
        # We can detect the condiction when the ball should rebound (when it touches the racket)   #
        # Using the `rebound` signal allows us to change the direction of the ball, and when no    #
        # rebound has been detected, set the self.two_scored or self.one_scored signals.           #
        #                                                                                          #
        ############################################################################################
        ...
        with m.If(moving):
            ...
            with m.If(timer == 0):
                ...
                #                             Write your HDL here                                  #
                #                                                                                  #
                # Check for rebound and act accordingly.                                           #
                #                                                                                  #
                ####################################################################################
                with m.Else():
                    m.d.sync += timer.eq(timer.reset),
